...
namespace domain.eventmetadata.service
{
  public interface IEventService
  {
    ...
    /// <returns></returns>
    List<BriefEventDto> SearchEvents(string search, string sort, EventType type, int page, int size);
    ...
    /// <param name="eventDto">이벤트 관리에 사용되는 객체</param>
    void SaveTempEvent(long adminId, EventDto eventDto);
    ...
    EventDto GetTempEvent(long adminId);

    /// <summary>
    /// 임시 저장된 이벤트 정보를 제거한다.
    /// </summary>
    /// <param name="adminId">이벤트를 임시로 저장한 관리자의 ID</param>
    void ClearTempEvent(long adminId);
    ...
    List<EventSearchHintDto> SearchHints(string search);
